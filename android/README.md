...
`libnode.so` is a build artifact, not checked-in source, so it has to exist
before Gradle can compile the JNI bridge against it:
...
it does not fall back to a build without a runtime.
...
| Foreground service in `:noderuntime` | PRD commit 8 |
| Watchdog / binder death recipient | PRD commit 9 |
| Bundle loader, boot receiver, Home + Advanced screens | PRD commits 10-13 |
